...
En este caso, el decorador @dataclass se encarga de crear automáticamente los métodos __init__, __repr__, __eq__ y __hash__.

Sintaxis: @nombre_decorador se coloca justo ANTES de la función/clase
"""

"""Importamos el decorador @dataclass de la librería dataclasses de Python."""
...
class PersonaSinDecorador:
    def __init__(self, nombre: str, edad: int, ciudad: str): # 'init' es un método especial utilizado para crear una instancia a partir de una clase. 
        self.nombre = nombre
        self.edad = edad
        self.ciudad = ciudad
    
    def __repr__(self): # 'repr' significa 'representación'. Es un método especial utilizado para mostrar el objeto de una forma legible, en formato de cadena de texto.
    ...
    def __eq__(self, other): # 'eq' significa 'igualdad'. Es un método especial utilizado a la hora de comparar dos objetos en cuanto a su contenido. Devuelve un valor booleano. 
    ...
    def __hash__(self): # Un 'hash' es un número único, valor entero, generado por Python, que identifica un objeto en concreto.
        ...
        # Entre dos objetos 'Persona' que tengan los mismos atributos, y estos atributos los mismos valores, sendos hashes serán idénticos.
        # Pero si se añade un nuevo atributo a uno de los objetos, o se modifica alguno de los atributos ya existentes, el hash del objeto 'Persona' cuyo atributo haya sido modificado
        # se recalculará, por lo que sendos hashes serán diferentes.
        # Si no se declara este método (en caso de no invocar el decorador @dataclass) o si no se le pasa este parámetro al decorador dentro de paréntesis con valor 'True',
...
@dataclass(frozen=True)  # 'Frozen' es un parámetro que hace que el objeto sea inmutable, es decir, que no se pueda modificar después de crearlo. Si se intenta modificar algún atributo, se generará un error.
